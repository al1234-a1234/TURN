-- ════════════════════════════════════════════════════════════════════════════
--  التحقّق من الصحّة تحت التزامن — يُنفّذ بعد كل درجة
--
--  نظامٌ سريعٌ يعطي نتائج خاطئة أسوأ من نظامٍ بطيءٍ صحيح. السرعة تظهر في k6،
--  والصحّة لا تظهر إلا هنا.
--
--  ⛛ القاعدة: **كل صفٍّ يرجع من هذا الملفّ = عطبٌ في التزامن.**
--     إن رجع صفٌّ واحد، توقّف عن التصعيد وسجّل الدرجة السابقة كسقفٍ آمن.
--
--  ⚠ درسٌ من الدرجة ١٠: كل مطعمٍ مبذورٍ (01_seed.sql) يحمل فرعًا اسمه
--  الحرفيّ «الفرع الرئيسي» وربما «الفرع الثاني» — الاسم نفسه لِمئة مطعمٍ
--  مختلف. التجميع بـb.name وحده يخلط فروعًا مختلفةً تمامًا في دلوٍ واحد
--  فيُظهر «تكرارًا» وهميًّا (رأينا هذا فعليًّا: ١٤ صفًّا بدت تكرارًا، وكل
--  واحدٍ منها فرعٌ مختلفٌ لمطعمٍ مختلف، برهانه joined_at متباعدة بالدقائق
--  لا المللي‌ثانية). **كل تجميعٍ هنا الآن بـb.id — لا بـb.name وحده.**
-- ════════════════════════════════════════════════════════════════════════════

-- (١) رقم ترتيبٍ مكرّر بين صفّين حيّين في نفس الفرع (بمعرّفه لا باسمه)
--     يعني أنّ القفل لم يمنع تسابق انضمامين
...
group by b.id, b.name, w."position"
having count(*) > 1

union all

-- (٢) شخصٌ أُجلِس وهو ملغى أو منتهِّ — انتقال حالةٍ غير مشروع نفذ من حارسه
...
-- (٤) صفّان حيّان لنفس العميل في نفس الفرع (بمعرّفه) — الفهرس الفريد خُرِق
select 'صفّان حيّان لعميلٍ واحد', b.name,
       w.customer_id::text, count(*)
from public.waitlist_entries w
join public.branches b on b.id = w.branch_id
where w.status in ('waiting','notified')
group by b.id, b.name, w.customer_id
having count(*) > 1
...
-- (٦) ترتيبٌ يخالف ترتيب الوصول — من جاء لاحقًا وترتيبه أقلّ (مقارنةٌ داخل
--     نفس branch_id أصلًا عبر شرط JOIN، فهذا الفحص لم يتأثّر بعطب الاسم)
...
-- ── ضابطٌّ موجب: لو رجع هذا فارغًا فالحمل لم يصل القاعدة أصلًا ──
