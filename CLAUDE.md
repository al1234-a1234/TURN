...
`supabase/migrations/0093_guest_writes_server_only.sql` **مكتوبٌ ولم يُطبَّق**.
يسحب دوالّ الكتابة من `anon` و`authenticated` فتصير الكتابة عبر خادمنا وحده.
والكود الذي يُناديها بمفتاح الخدمة (`lib/supabase/guest-writes.ts`) موجودٌ في
هذا الفرع ولم يُنشر بعد. **تطبيقه قبل النشر يكسر كلّ انضمامٍ حيّ فورًا.**

التسلسل الملزم:
1. يُدمج الفرع ويُنشر على الإنتاج.
2. يُفتح `/api/health` ويُتأكَّد من `"writer": true` — أي أنّ
   `SUPABASE_SERVICE_ROLE_KEY` حاضرٌ في بيئة النشر. بدونه يفشل كلّ انضمام.
3. يُطبَّق 0093، ثم تُضاف فحوصه إلى `critical_checks.sql`.

وقد جُرّب داخل معاملةٍ تُلغى على الإنتاج: ٢٣ تأكيدًا كلّها صحيحة (الكتابة
مغلقة عن الضيف والمسجَّل، الحجز يبقى للموظّف، وسبع دوالّ قراءةٍ يناديها
المتصفّح مباشرةً بقيت مفتوحة).
