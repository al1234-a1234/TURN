...
**ما يبقى على مستوى العلامة عمدًا**: `customers` و`customer_restaurant`
و`customer_rewards` و`loyalty_programs` — العميل وولاؤه ملك العلامة لا الفرع،
ونقاطه لازم تشتغل في أي فرع. لكن *سجلّ زياراته* في ملفّه يظهر مقصورًا على
فروع المتصل، لأن RLS على الطابور والحجوزات يصفّيه تلقائيًّا.
...
- **قاعدة البيانات**: `supabase/tests/critical_checks.sql` — ٣٤ فحصًا (صلاحيات،
