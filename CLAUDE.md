...
> **اقرأ [`ops/CHARTER.md`](ops/CHARTER.md) أوّل كلّ جلسة قبل أيّ عمل.** هذا
> الملفّ يشرح *كيف* يعمل النظام؛ والميثاق يشرح *ما يجوز فعله به*: التجميد
> السّاري وشرط رفعه، ومن يملك قرار كلّ طبقةٍ من الإصلاح، والخطوط الحمراء،
> وقاعدة «الدليل الحيّ أو (لم أتحقّق بعد)».
...
## ✅ 0093 مطبَّقٌ ومُغلق — كتابةُ الضيف عبر الخادم وحده

> **صُحِّح في ٣١ أغسطس ٢٠٢٦.** كان هنا تحذيرٌ يقول إنّ `0093` «مكتوبٌ ولم
> يُطبَّق» وإنّ «تطبيقه قبل النشر يكسر كلّ انضمامٍ حيّ فورًا». **الأمران
> غير صحيحَين اليوم**، والإبقاء عليهما كان يدفع أيّ جلسةٍ قادمةٍ إلى قرارٍ
> خاطئ: إمّا الامتناع عن إصلاحٍ لازم، أو إعادةُ فتحِ ما هو مغلق.

التسلسل تمّ فعلًا وسجلّ الترحيلات يشهد به بالترتيب:
`guest_writes_server_only` ← `rollback_0093_guest_writes` ← `guest_writes_server_only_v2`.
أي أنّه طُبِّق، ثمّ رُوجع، ثمّ أُعيد تطبيقه بنسخةٍ ثانية — وهي القائمة الآن.

**الدليل الحيّ** (`has_function_privilege` على الإنتاج، ٣١ أغسطس ٢٠٢٦):

| الدالّة | `anon` | `authenticated` | `service_role` |
|---|---|---|---|
| `join_waitlist_guest` | ✗ | ✗ | ✓ |
| `cancel_by_ticket` | ✗ | ✗ | ✓ |
| `confirm_attendance` | ✗ | ✗ | ✓ |
| `submit_review` | ✗ | ✗ | ✓ |
| `guest_status_by_phone` (الصيغتان) | ✗ | ✗ | ✓ |
| `tv_queue` | ✓ | ✓ | ✓ |
| `waitlist_ticket_status` | ✓ | ✓ | ✓ |

كلّ كتابةٍ للضيف تمرّ بخادمنا (`lib/supabase/guest-writes.ts`) بمفتاح الخدمة،
ودالّتا القراءة اللتان يناديهما المتصفّح مباشرةً باقيتان مفتوحتين بالتصميم.

و`/api/health` يردّ حيًّا `{"ok":true,"db":"ok","writer":true,"cron_fresh":true}`
— فـ`SUPABASE_SERVICE_ROLE_KEY` حاضرٌ في بيئة النشر، وهو الشرط الذي كان
التحذير القديم يخشى غيابه.
