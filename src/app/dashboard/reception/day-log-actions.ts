 ...
 * `restore_queue_entry` دالّة SECURITY DEFINER تفحص `my_managed_branch_ids()`
 * بنفسها وترفع 42501 لغير المخوَّل. فالفحص هنا **دفاعٌ في العمق فقط**، لا
 * الحارس الوحيد: نداءٌ مباشر عبر PostgREST يتخطّى الواجهة كلّها، وهو بالضبط
 * ما يجعل إخفاءَ الزرّ حمايةً وهميّة (عطل ث-٢ في 0106).
 *
...
> {
  ...
  if (error) {
    ...
    const byCode: Record<string, string> = {
      "42501": "الإرجاع لمالك المطعم أو مديره فقط.",
