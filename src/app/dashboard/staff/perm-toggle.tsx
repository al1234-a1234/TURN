...
export function PermToggle({
  ...
  return (
    ...
    >
      ...
      <span className="relative h-6 w-11 shrink-0 rounded-full transition" style={{ background: on ? "var(--brand-solid)" : "var(--surface-2)", border: "1px solid var(--border)" }}>
        <span className="absolute top-1/2 h-4 w-4 -translate-y-1/2 rounded-full bg-[color:var(--surface)] shadow transition-all" style={{ insetInlineStart: on ? "1.45rem" : "0.2rem" }} />
      </span>
    </button>
  );
}
