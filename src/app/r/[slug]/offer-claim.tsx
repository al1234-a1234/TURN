  ...
  if (!open) {
    return (
      <button type="button" onClick={() => setOpen(true)}
              className="shrink-0 rounded-xl px-3.5 py-2 text-xs font-extrabold text-white"
              style={{ background: "linear-gradient(150deg,#b23c1d,#661c0a)" }}>
        {tr(lang, "استخدمه", "Claim")}
      </button>
    );
  }
    ...
    <div className="shrink-0">
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          onClick={() => start(async () => setState(await claimOffer(offerId, phone)))}
