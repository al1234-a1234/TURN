...
export default async function RestaurantPublicPage({
  ...
  const { data: branches } = await supabase
    .from("branches").select("id, name, city, address, branch_settings(accepts_waitlist, accepts_reservations, manually_closed, busy_now, queue_paused, join_frozen, join_frozen_reason, opening_hours, max_party_size, max_waitlist_size), branch_zones(key, name, name_en, sort_order, is_active)")
  ...
  const withCounts = branchList.map((b) => {
    ...
    const settings = bs as { accepts_waitlist?: boolean; accepts_reservations?: boolean; manually_closed?: boolean; busy_now?: boolean; queue_paused?: boolean; join_frozen?: boolean; join_frozen_reason?: "done_today" | "temporary" | null; opening_hours?: { open?: string; close?: string } | null; max_party_size?: number; max_waitlist_size?: number | null } | null;
    ...
    return {
      ...
      joinFrozenReason: settings?.join_frozen_reason ?? null,
      ...
      maxWaitlistSize: settings?.max_waitlist_size ?? null,
