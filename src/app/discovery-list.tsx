...
import { useViewTransitionNav } from "@/lib/use-view-transition-nav";
...
export type DiscoveryItem = {
  ...
  branchCount: number;
  /**
   * إيقاف الانضمام الذي ضغطه الاستقبال — `null` حين لا إيقاف. القيمة نفسها
   * التي يكتبها `set_branch_join_frozen` ويقرؤها الضيف داخل صفحة المطعم؛ لا
   * حالة جديدة ولا عمود جديد، عرضٌ لحالةٍ قائمة على القائمة العامّة.
   */
  frozenReason: "done_today" | "temporary" | null;
 ...
 *
 * كان سطر الحالة كلّه ۳۰px/۶۰۰ — أي أن «7» في «داخلي 7» له وزنُ حرفٍ في
...
/**
 * حالة البطاقة: سطرٌ نصٍّ واحد يحمل لونه — لا بنر ولا كبسولات.
 *
...
type CardState = {
  ...
  chip: string | null;
  tone: Tone;
  /** التفصيل تحت الاسم: «۳ داخلي · ۲ خارجي» — خبرٌ لا حالة */
  parts: string[];
};
...
function cardState(r: DiscoveryItem, lang: Lang): CardState {
  ...
  //
  // وبعنابيّ الهويّة لا رمادي: جرّبتُ الرمادي خوفًا من التباسه بعنابيّ
  ...
  // مغلقٌ **و**موقوف الانضمام: «مغلق» يتقدّم، وهو قرارٌ لا تسويةٌ بينهما.
  // سببان: (١) «مغلق» يجيب سؤال العميل كاملًا — لا دخول ولا جلوس ولا دور —
  // بينما «اكتملت حجوزات اليوم» تصف منعَ الجديد وحده، وهو جزءٌ ممّا يقوله
  // الإغلاق أصلًا؛ فلا خبر يُفقد. (٢) الإيقاف لا يُلغى تلقائيًا بينما
  // الإغلاق يأتي من الساعة: فرعٌ أُوقف أمس ثمّ خرج عن دوامه اليوم يعرض
  // رسالةً بائتة تصرف العميل عن مطعمٍ سيفتح بعد ساعة. ولأن المغلقين يُجمعون
  // تحت عنوان «مغلق حاليًا»، فكبسولة «اكتملت الحجوزات» داخله تُقرأ تناقضًا:
  // «مغلق» تحت العنوان و«ممتلئ» في الكبسولة توحيان أنه يعمل وامتلأ.
  if (r.frozenReason) {
    return {
      // النصّ حرفيّ كما يقرؤه الضيف في صفحة المطعم (waitlist-form.tsx:813)
      // — ولا تُصاغ له نسخةٌ «مختصرة للكبسولة»: شاشتان تقولان الشيء نفسه
      // بلفظين تُقرآن حالتين.
      chip:
        r.frozenReason === "done_today"
          ? tr(lang, "اكتملت حجوزات اليوم", "Today's bookings are full")
          : tr(lang, "المطعم مزدحمٌ حاليًا", "We're busy right now"),
      // نفس نبرة «مغلق الآن» تمامًا: كبسولةٌ هادئة بحدٍّ لا مساحةٌ مصمتة
      // تنادي — لا فعل للعميل هنا، فلا تُنادى عينُه إليها.
      tone: "closed",
      parts: [],
    };
  }
  ...
  // «۳ عوائل · ۲ أفراد». العميل يسأل «أين أجلس؟» لا «كم العدد؟»، والاسم
...
const CHIP_STYLE: Record<Tone, React.CSSProperties> = {
  // المغلق لا فعل فيه: كبسولةٌ هادئة بحدٍّ لا مساحةٌ مصمتة تنادي
  closed: { background: "var(--surface-2)", color: "var(--brand-d)", border: "1px solid var(--border)" },
  // كانت --st-open (أخضر) هنا، ثم جُرّب الذهبيّ (--star) — كلاهما رُفض:
  // المشغّل صريح أن هويتنا عنابيّةٌ لا ذهبية. فصار عنابيًا فاتحًا (شفافية
  // خفيفة من نفس رقم الهوية، لا لونًا ثانيًا) — يبقى مميّزًا عن «مشغول»
  // العنابي المصمت بخفّة الامتلاء لا باختلاف الدرجة، وعن «مغلق» بغياب
  // الحدّ. ثلاثتها عنابيّ، يتفاوت وزنه: شفاف خفيف / حدٌّ فقط / مصمتٌ كامل.
...
function Card({ r, lang }: { r: DiscoveryItem; lang: Lang }) {
  ...
  // الانتقال البصريّ (fade + انزلاق خفيف) عبر View Transitions API الأصليّة
  // — انظر src/lib/use-view-transition-nav.ts للتعليل الكامل. الـhref يبقى
  // كما هو فيبقى الـprefetch شغّالًا تمامًا (أثبتناه ١٠/١٠ على القياس
  // الفعليّ)؛ فقط الضغطة العاديّة تتحوّل لتنقّلٍ برمجيٍّ ملفوفٍ بالانتقال.
  ...
  return (
    <Link
      href={`/r/${r.slug}`}
      onClick={(e) => {
        storePeek(r.slug, { name: r.name, logo: r.logo_url, waiting: r.waiting, closed: r.closedNow });
        // فتحٌ بتبويبٍ جديد أو نافذة (ctrl/cmd/shift/عجلة الفأرة) يبقى
        // بسلوكه الطبيعي — الانتقال البصريّ لتصفّح هذا التبويب وحده.
    ...
    >
      {/* البلاطة بيضاء بحدّ شعرة: الشعار يضعه صاحب المطعم، فلا نصبغه بلوننا.
          ۷۲ لا ۶۴: طلب المالك بلاطةً أكبر شوي («أكبر شوي» — درجةٌ لا قفزة).
          وهذا هو المقاس الذي وثّق تعليقٌ سابقٌ هنا رفضَه («عند ۷۲ يتجاوز
          ارتفاعُ البلاطة كتلةَ الأسطر الثلاثة فيبقى فراغٌ ميّت أسفل النصّ»)
          — أُبقيه موثّقًا هنا لأنّ الحساب الذي بُني عليه ما زال صحيحًا
          بأحجام الخطّ الحاليّة: بطاقةٌ هادئة (اسمٌ+فروع+مطبخ، بلا طابور)
          نصُّها ≈۶۴px فتتجاوزه البلاطة بنحو ١٠px. وفي المقابل بطاقةٌ
          مزدحمة (+سطر توزيع الطابور) نصُّها ≈۸۵px فتقترب البلاطة منه. لم
          يُختبر بصريًّا من هذه الجلسة — طلبٌ صريح من المالك تجاوز الرفض
          السابق، فلينظر بنفسه بعد النشر.
          والانحناء ١٢ لا دائرة: جُرّبت الدائرة هنا فرآها المشغّل «غير جيدة»
          وطلب إرجاعها صراحةً — البلاطة المربّعة أهدأ داخل صفٍّ انحناؤه ١٤. */}
      <span
      ...
      <div className="min-w-0 flex-1">
        ...
        {/* سطران لا سطر: الفروع والمطبخ خبران مختلفان — «أين؟» و«ماذا؟» —
            ...
            وبعنابيٍّ واحدٍ غير مخفّف: كنتُ أُخفّت المطبخ إلى ٠٫۷۲ لأرتّبه
            تحت الفروع، فبدا لونًا ثالثًا شاحبًا لا درجةً أخفت. والترتيب
            يحمله وزنُ الخطّ (semibold ثم medium) بلا أن يُمسّ اللون. */}
      ...
      {/* الطرف الأيسر: التقييم فوق الحالة — كلاهما حكمٌ على المطعم لا وصفٌ
          ...
          مجتمعين في المنتصف عموديًا كما كانوا — طلبٌ صريح بعد لقطة شاشة. */}
...
function SectionHeading({ label, count }: { label: string; count: number }) {
  return (
    // ‎-mx-1 يلغي ۴px من حشو `main` (۲۰) فيبدأ العنوان عند ١٦ — نفس الخطّ
...
export function DiscoveryList({ items }: { items: DiscoveryItem[] }) {
  ...
  // المسافة حُذفت من البطاقة، ومعها قراءة إذن الموقع التي كانت تغذّيها وحدها:
  ...
  // الآن» — سطران متناقضان في صفٍّ واحد. والكبسولة نفسها كانت سليمة (فرعُ
  ...
  // نادر — الإيقاف يصاحبه طابورٌ ممتلئٌ عادةً فيقع الصفّ في «فيه طابور» أصلًا
  // — لكنّ ندرته لا تجعله مقبولاً: «متاح الآن» دعوةٌ للحضور، والموقوف يردّ
  // من يحضر.
  ...
  // كان الفرعان يعيدان نفس التنسيق حرفيًّا، فالمطبخ المختار لا يتميّز عن
  ...
  // حالة الفراغ انتقلت إلى هنا من الصفحة: نصُّها مترجمٌ، وبقاؤه
      ...
      {groups.length === 0 ? (
        ...
        groups.map((g) => (
          <section key={g.key}>
            ...
            {/* كتلةٌ واحدة لكل قسم: إطارٌ واحد وظلٌّ واحد، والصفوف موصولة
