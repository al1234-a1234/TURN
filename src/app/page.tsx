...
// تُخزَّن على الحافة ٦٠ ثانية فتصل للزائر جاهزةً بدل توليدٍ كامل لكل طلب.
// وأرقام الطابور الحيّة ليست هنا: التذكرة وصفحة المطعم والاستقبال بلا كاش.
//
// ‏٦٠ لا ٣٠: التقادم الزمنيّ ليس ما يُحدّث هذه الصفحة أصلًا. كلّ انضمامٍ
// وإلغاءٍ وتجليسٍ يستدعي `revalidateTag("queue-counts")` و`revalidatePath("/")`
// فورًا (r/[slug]/actions.ts:230-231 · dashboard/waitlist-actions.ts:83)،
// وكذلك كلّ تبديلٍ لحالة الفرع من الاستقبال (status-actions.ts). فالثواني
// هنا شبكةُ أمانٍ خلف إبطالٍ صريح.
//
// وما لا إبطالَ صريحَ له هو ما يتأثّر بالتكبير: تغيّر «مفتوح/مغلق» بحسب
// ساعات الدوام — لا فعلَ بشريًّا يرافقه، فيتأخّر ظهوره حتى ٦٠ث بدل ٣٠ث.
// وهو مقبول: الفرق نصف دقيقةٍ على لافتةٍ تتغيّر مرّتين في اليوم.
//
// وعدّادا الطابور (`queue-counts`) بقيا على ١٠ث عمدًا ولم يُمسّا — تعليقهما
// في public-cache.ts يوثّق شكوى مشغّلٍ حقيقية على التناقض بين الشاشتين.
