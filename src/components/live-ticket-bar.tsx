...
const TTL_MS = 15_000;
// ‏٦٠ث لا ١٢: الملفّ كان يقول «مرّةً كل دقيقة» في موضعين والثابت ١٢ ألفًا —
// خمسة أضعاف ما وُثّق. والشريط سطحٌ محيط لا شاشةَ متابعة: من يراقب دوره فعلًا
// على التذكرة (٤/١٠/٢٠ث بـjitter) لا هنا. والتنقّل بين الصفحات يُنعشه أصلًا
// كل ١٥ث عبر الكاش، وعودةُ التبويب و bfcache تُنعشانه فورًا — فالنبضة تحكم
// حالةً واحدة: جالسٌ ساكنٌ على صفحةٍ واحدة.
...
export function LiveTicketBar({ onShow }: { onShow?: (shown: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    // متجاوزًا الكاش، وعودة التبويب للواجهة تسأل باحترام مهلة الكاش (١٥ث)
    // لا فترةَ النبضة — وهي أقصر منها، فالعائد إلى تبويبه يرى حالةً حديثة
    // بلا انتظار الدورة. (كان مكتوبًا هنا «مهلة الدقيقة» وهو وصفٌ خاطئ:
    // ‏load(false) يقرأ readCache وTTL_MS، ولا علاقة له بـPOLL_MS.)
