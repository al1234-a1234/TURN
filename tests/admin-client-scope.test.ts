 ...
 *
 * `supabase/admin.ts` يحمل مفتاحًا يتجاوز كل سياسات RLS — أي ملفٍّ يستورده يصير
 * قادرًا على قراءة كل مطعمٍ وكل عميل. و`server-only` يمنع وصوله إلى المتصفّح،
 * لكنه لا يمنع توسّعه داخل الخادم: يستورده أحدهم «مؤقتًا» لتجاوز RLS مزعجًا،
...
const ALLOWED = new Set([
  // إرسال إشعارات الطابور: دواله سُحبت من anon كي لا تُسرّب بيانات الاشتراك
  join("lib", "push.ts"),
  ...
  // الملفّ هو المنفذ الوحيد لها، فيبقى التوسّع في ملفٍّ واحدٍ لا في عشرة.
  join("lib", "supabase", "guest-writes.ts"),
  // نبض الصحة: يُثبت أنّ المفتاح مفتاح خدمةٍ بمسبارٍ لا يملكه غيره —
  // وهو الشرط الذي بُني عليه سحب دوالّ الكتابة من الضيف (0093).
  join("app", "api", "health", "route.ts"),
  // أذن بوت تلغرام: أمر المالك يعبر المستأجرين (يقرأ كل المطاعم ويكتب في
  // إعداداتها) بلا جلسة مستخدم — فلا يوجد «عميل مستدعٍ» يحكمه RLS أصلًا.
  // والمنفذ ضيّقٌ عمدًا: نداء telegram_command وحدها، وهي مسحوبةٌ من anon
  // و authenticated ويحرسها w20_telegram_cmd_locked في شبكة الفحوص.
  join("app", "api", "telegram", "webhook", "route.ts"),
  // حدّ محاولات دخول الموظّفين/الملّاك: check_rate() مسحوبة من anon/authenticated
  // (SECURITY DEFINER بلا EXECUTE عام)، فعميل المستدعي في صفحتي partners/reception
  // (قبل تسجيل الدخول، لا جلسة أصلًا) لا يكفي لمناداتها. منفذٌ واحد لصفحتي الدخول
  // كلتيهما، كي لا يتكرر استيراد مفتاح الخدمة في كل صفحة دخول.
  join("lib", "login-rate-limit.ts"),
  // الوحدة نفسها
...
test("عميل الخدمة لا يُستورد إلا من المسارات المصرّح بها", () => {
