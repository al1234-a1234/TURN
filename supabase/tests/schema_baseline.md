...
| وظائف الكرون | ٧ |
| دوال `SECURITY DEFINER` متاحة لـ `anon` | ٧ |
...
كحدٍّ أعلى — كان ٣٠ لمّا كان الواقع ٢١، وشُدَّ إلى ٧ (0111-0114، انظر أسفله)
مطابقًا للواقع الجديد. أي دالة جديدة تُكشف للضيف بلا داعٍ تُسقط الفحص فورًا.

آخر تحديثٍ للبصمة: **0115**. ما غيّرها منذ 0107:
...
داخلية كانت مكشوفة بلا داعٍ (`is_staff_of`، `is_platform_admin`،
`staff_has_perm`، ...) — GRANT/REVOKE لا يغيّر عدد الدوال نفسه، لكنه
غيّر عمود «متاحة لـanon» من ٢١ إلى ٧؛
`0115` أضاف دالّةً واحدة (`check_platform_health`) — ١٠١ صارت ١٠٢.
