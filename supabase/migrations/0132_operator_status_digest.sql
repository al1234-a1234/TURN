-- ============================================================================
--  ملخّص الحالة الدوري على تيليجرام — طلب مباشر: نظام send_platform_alerts
--  القائم (0116-0120، كل ٥ دقائق) صامتٌ عمدًا ما لم يتغيّر شيء — وهذا صحيحٌ
--  هندسيًّا (لا فيضان رسائل) لكنه يترك المشغّل بلا أي إشارة أن كل شيء بخير.
--  هنا إضافةٌ مختلفة الغرض: رسالة "الحالة الآن" حتى وهي كلّها ✓، كل نصف
--  ساعة، ومرّة كل ساعة تُرفَق بملخّص شبكة الفحوص الحرجة (critical_checks.sql)
--  — التي كانت حتى الآن سكربتًا يُشغَّل يدويًّا فقط عبر Supabase MCP. تحويلها
--  إلى دالّةٍ مخزَّنة (run_critical_checks) يخدم غرضين: يتيح تشغيلها آليًّا هنا،
--  ويبقيها قابلة الاستدعاء يدويًّا للتحقق بعد أي نشرٍ مستقبلي أيضًا.
-- ============================================================================

-- ── ١) شبكة الفحوص الحرجة كدالّة مخزَّنة (نسخة طبق الأصل من critical_checks.sql) ──
...
as $function$
  with checks(name, pass) as (
    ...
    -- ══ 0132: ملخّص الحالة الدوري — الدوال محجوبة عن anon/authenticated، وكرونها حيّ ══
    ...
    ('q20_schema_no_drift',      (select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
                                 ...
                                 -- 0132: +٢ دالة (run_critical_checks، send_platform_status_digest) — ١٣١ صارت ١٣٣
...
revoke all on function public.run_critical_checks() from public;
revoke all on function public.run_critical_checks() from anon, authenticated;

-- ── ٢) رسالة الحالة الدورية على تيليجرام ──
-- كل نصف ساعة: سطرٌ مختصر (زمن استجابة الصفحتين، حالة الطابور، اتصالات
-- القاعدة، أخطاء العملاء الأخيرة) — بصرف النظر إن كان كل شيء سليمًا أم لا،
-- خلافًا لـsend_platform_alerts التي تصمت عمدًا عند السلامة. وفي الساعة
-- المضبوطة تحديدًا (الدقيقة صفر) يُرفَق أيضًا ملخّص شبكة الفحوص الحرجة —
-- عدد الناجح من الإجمالي، وأسماء أي فحصٍ فاشل إن وُجد (نادر: يتغيّر فقط
-- بعد نشر كودٍ جديد، لا بحركة اليوم العادية).
...
revoke all on function public.send_platform_status_digest(boolean) from public;
revoke all on function public.send_platform_status_digest(boolean) from anon, authenticated;

-- ── ٣) الجدولة: كل نصف ساعة — والدالّة نفسها تُرفِق الملخّص الكامل عند
--       الدقيقة صفر (أي: مرّة كل ساعة تحديدًا، كما طلب المشغّل) ──
select cron.schedule(
  'operator-status-digest',
  '*/30 * * * *',
  $$select public.send_platform_status_digest(extract(minute from now())::int = 0)$$
);
