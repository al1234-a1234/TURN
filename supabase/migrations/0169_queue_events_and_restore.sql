...
--  بلا مخرج.
--
--  ════════ القرار (ب): الإرجاع لصلاحية waitlist — حسمه المالك ════════
--  المُطبَّق: `my_branch_ids_for('waitlist')` — أي موظّف الاستقبال بصلاحيّته
--  العاديّة، لا مالك/مدير فقط.
--
--  طُرح السؤال أوّلًا بالافتراضيّ الأضيق (`my_managed_branch_ids()`) قياسًا
--  على ٠١٠٦ الذي صعّد **الحذف** وحده. وردّ المالك حسم الأمر بحجّةٍ تشغيليّة
--  تسبق القياس: من يكتشف الخطأ هو من يقف على الباب لحظتَه — والمالك نائم.
--  فحصرُ التصحيح في المدير يعني تعطّله حتّى يصل، وحينها تكون نافذة الإرجاع
--  (١٥ دقيقة) قد مضت أصلًا، فتصير الميزة موجودةً على الورق معطّلةً في الواقع.
--
--  وما يبقى حارسًا رغم التوسيع:
--   • الصلاحية مفروضةٌ **داخل الدالّة** لا في الواجهة — نداءٌ مباشر عبر
--     PostgREST لا يتخطّاها (عطل ث-٢ في ٠١٠٦).
--   • حصرُ الفرع قائم: `my_branch_ids_for` تُرجع فروعَ المتصل وحدها، فموظّف
--     فرعٍ لا يُرجع في فرعٍ آخر ولو من نفس المطعم.
--   • نافذة ١٥ دقيقة، ومنعُ التكرار، وكلّ حركةٍ مُوقَّعةٌ باسم منفّذها في
--     `queue_events.actor` — فالتوسيع لم يُلغِ المساءلة.
--   • و`w44` أدناه يحرس القيمة الجديدة تحديدًا: يسقط إن عاد أحدٌ إلى
--     `my_managed_branch_ids` صامتًا.
--
...
begin
  ...
  -- والمستوى `my_branch_ids_for('waitlist')` بقرار المالك: من يقف على الباب
  -- لحظةَ الخطأ هو من يصحّحه — انظر القرار (ب) في رأس الملفّ.
...
begin
  ...
  if position('w41_queue_log_trigger' in d2) = 0 then
    d2 := replace(d2, E'    (\'q20_schema_no_drift\',',
      ...
      || E'    (\'w44_restore_waitlist_perm\',(select pg_get_functiondef(oid) like \'%my_branch_ids_for%\'\n'
      || E'                                    and pg_get_functiondef(oid) not like \'%my_managed_branch_ids%\'\n'
