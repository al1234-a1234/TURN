-- تحديث خطّ أساس q20_schema_no_drift بعد إضافة دوال 0208/0209 المتعمَّدة:
-- الدوال العامّة صارت ١٤٨ (كانت ١٤٥) لإضافة dashboard_customer_kpis و
-- customers_page_counts و customers_search_count. وسياسات public أصبحت ٧٤
-- (كانت ٧٣) — فارقٌ سابقٌ لهذه الجلسة لم يُحدَّث الخطّ من أجله بعد؛ الأعداد
-- الأربعة تحقّقتُ منها حيّةً على الإنتاج قبل الكتابة (بلا CREATE) فتطابق
-- الواقع الحاليّ تمامًا: جداول ٣٦، دوال ١٤٨، سياسات ٧٤، مفاتيح أجنبية ٤٤.
  ...
  with checks(name, pass) as (
    ...
    ('q20_schema_no_drift',      (select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
                                 ...
                                 and (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
                                      where n.nspname='public' and p.prokind='f') = 148
